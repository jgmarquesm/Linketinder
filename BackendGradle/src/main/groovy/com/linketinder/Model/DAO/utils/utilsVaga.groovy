...
class utilsVaga {
    ...
    static ArrayList listarVagas(ConexaoDB conexaoDB) {
        try {
            Sql read = conexaoDB.conectar()
            ArrayList<String> lista_habilidades = new ArrayList<>()
            ArrayList vagas = new ArrayList<>()
            read.eachRow("""SELECT v.id, v.nome, v.descricao, v.senioridade, v.cidade FROM vagas AS v""") { row ->
                ...
                def vaga = ["id": row.getString("id"), "nome": row.getString("nome"), "descricao": row.getString("descricao"), "senioridade": row.getString("senioridade"), "cidade": row.getString("cidade"), "habilidades": lista_habilidades]
                vagas.add(vaga)
                lista_habilidades = []
            }
